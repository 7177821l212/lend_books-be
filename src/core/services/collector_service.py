...
class CollectorService:
    ...
    async def create(
        self, current_user: dict, body: CollectorCreate
    ) -> CollectorResponse:
        self._require_investor(current_user)
    ...
    async def activate(
        self, current_user: dict, collector_id: str
    ) -> CollectorResponse:
        self._require_investor(current_user)
        result = await self.session.execute(
            select(User).where(
    ...
    async def delete(self, current_user: dict, collector_id: str) -> None:
        ...
        if int(active_count) > 0:
            ...
            raise ConflictError(
                "reassign or close active loans before deleting this collector"
            )
    ...
    async def report_location(self, current_user: dict, body: LocationReport) -> None:
        ...
        result = await self.session.execute(
            select(CollectorLocation).where(
                CollectorLocation.collector_id == collector_id
            )
        )
        row = result.scalar()
        # The device timestamp is useful only for diagnostics. Store server time so
        # the investor's "live" indicator cannot be distorted by a wrong device clock.
    ...
    async def list_locations(
        ...
        """Investor-only last-known locations for active collectors."""
        self._require_investor(current_user)
        result = await self.session.execute(
